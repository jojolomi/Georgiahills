  ...
  const UI = {
    ...
    standardizeDestinationLayout() {
      ...
      if (highlights && !contentCard.querySelector('.itinerary-block')) {
        ...
        itineraryBlock.style.cssText = 'margin-top:2rem; padding:1.5rem;';
        const heading = document.createElement('h3');
        heading.className = 'section-heading';
        heading.textContent = isArabic ? 'أبرز ما في الرحلة' : 'Trip Highlights';
        itineraryBlock.appendChild(heading);
        itineraryBlock.appendChild(highlights.cloneNode(true));
        contentCard.appendChild(itineraryBlock);
      }
    }
  };
